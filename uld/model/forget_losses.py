...
def _first_model_device(module):
    hf_map = getattr(module, 'hf_device_map', None)
    if hf_map:
...
def _attach_grad_anchor_if_needed(loss, model):
    if loss.requires_grad:
        return loss
    grad_anchor = next((p for p in model.parameters() if p.requires_grad and p.numel() > 0), None)
    if grad_anchor is None:
        return loss
    return loss + grad_anchor.reshape(-1)[0] * 0.0

def NextTokenPredictionLoss(model: AutoModelForCausalLM, input_ids, attention_mask, labels, position_ids=None):
    # In model parallel mode, ensure inputs are on the first device of the model
    if hasattr(model, 'hf_device_map'):
        first_device = _first_model_device(model)
        input_ids = _move_to_device(input_ids, first_device)
        attention_mask = _move_to_device(attention_mask, first_device)
        labels = _move_to_device(labels, first_device)
        if position_ids is not None:
            position_ids = _move_to_device(position_ids, first_device)
...
class ForgetRetainLoss:
    def __init__(self, forget_loss_func, retain_loss_func=None, retain_weight=1.0, forget_weight=1.0) -> None:
    ...
    def calculate_loss(self, model, input_ids, attention_mask, labels=None, retainlabels=None, oracle_model=None, **kwargs):
        ...
        if forget_input_ids is None or forget_input_ids.shape[0] == 0 or self.forget_loss_func is None:
        ...
        if retain_input_ids is None or retain_input_ids.shape[0] == 0 or self.retain_loss_func is None:
        ...
        loss = _attach_grad_anchor_if_needed(loss, model)
...
class PreferLoss(ForgetRetainLoss):
    ...
    def calculate_loss(self, model, input_ids, labels, label_attention_mask, prefer_input_ids, prefer_labels, prefer_label_attention_mask, retainlabels, oracle_model=None, **kwargs):
        ...
        if retain_input_ids.shape[0] == 0 or self.retain_loss_func is None:
...
def KLLossFunc(model, input_ids, attention_mask, labels, oracle_model, **kwargs):
    oracle_device = _first_model_device(oracle_model)
    oracle_input_ids = _move_to_device(input_ids, oracle_device)
    oracle_attention_mask = _move_to_device(attention_mask, oracle_device)
    with torch.no_grad():
        retain_outputs = oracle_model(
            oracle_input_ids, attention_mask=oracle_attention_mask, use_cache=False
        )
        retain_probs = F.log_softmax(retain_outputs.logits.float(), dim=-1)
        retain_probs = retain_probs.view(-1, retain_outputs.logits.shape[-1])

    outputs = model(input_ids,labels=labels, attention_mask=attention_mask)
    probs = F.log_softmax(outputs.logits.float(), dim=-1)
    probs = probs.view(-1, outputs.logits.shape[-1])
    if retain_probs.device != probs.device:
        retain_probs = retain_probs.to(probs.device)
    retain_loss = nn.functional.kl_div(probs, retain_probs, reduction='batchmean', log_target=True)
    return retain_loss

def NegKLLossFunc(model, input_ids, attention_mask, labels, oracle_model, **kwargs):
    # Maximize KL divergence from the oracle model on the given split.
    return -1 * KLLossFunc(
        model, input_ids=input_ids, attention_mask=attention_mask, labels=labels, oracle_model=oracle_model, **kwargs
    )
...
def DPOLossFunc(model, input_ids, labels, label_attention_mask, prefer_input_ids, prefer_labels, prefer_label_attention_mask, oracle_model, beta=0.1, **kwargs):
    oracle_device = _first_model_device(oracle_model)
    oracle_input_ids = _move_to_device(input_ids, oracle_device)
    oracle_label_attention_mask = _move_to_device(label_attention_mask, oracle_device)
    oracle_labels = _move_to_device(labels, oracle_device)
    oracle_prefer_input_ids = _move_to_device(prefer_input_ids, oracle_device)
    oracle_prefer_label_attention_mask = _move_to_device(prefer_label_attention_mask, oracle_device)
    oracle_prefer_labels = _move_to_device(prefer_labels, oracle_device)

    with torch.no_grad():
        oracle_origin_outputs = oracle_model(
            oracle_input_ids, attention_mask=oracle_label_attention_mask, use_cache=False
        )
        oracle_prefer_outputs = oracle_model(
            oracle_prefer_input_ids, attention_mask=oracle_prefer_label_attention_mask, use_cache=False
        )
        oracle_origin_loss = -1 * TokenNextTokenPredictionLoss(oracle_origin_outputs.logits, oracle_labels)
        orcale_prefer_loss = -1 * TokenNextTokenPredictionLoss(oracle_prefer_outputs.logits, oracle_prefer_labels)
    ...
    if orcale_prefer_loss.device != pi_logratios.device:
        orcale_prefer_loss = orcale_prefer_loss.to(pi_logratios.device)
    if oracle_origin_loss.device != pi_logratios.device:
        oracle_origin_loss = oracle_origin_loss.to(pi_logratios.device)
...
def NPOLossFunc(model, input_ids, attention_mask, labels, oracle_model, beta=0.1, **kwargs):
    ...
    oracle_labels = _move_to_device(labels, oracle_device)
    with torch.no_grad():
        oracle_outputs = oracle_model(oracle_input_ids, attention_mask=oracle_attention_mask, use_cache=False)
        oracle_logits = oracle_outputs.logits
        oracle_tokenloss = TokenNextTokenPredictionLoss(oracle_logits, oracle_labels)

    outputs = model(input_ids, attention_mask=attention_mask)
    logits = outputs.logits
    tokenloss = TokenNextTokenPredictionLoss(logits, labels)
    if oracle_tokenloss.device != tokenloss.device:
        oracle_tokenloss = oracle_tokenloss.to(tokenloss.device)
...
def create_unlearn_loss(loss_config):
    ...
    if forget_loss_func is not None and forget_loss_func.__name__ == 'DPOLossFunc':
        return PreferLoss(
        ...
        return ForgetRetainLoss(
            ...
            forget_weight=loss_config.get('forget_weight', 1.0),
        )
