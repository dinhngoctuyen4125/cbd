...
class ContrastGenerationMixin(GenerationMixin):
    ...
    def _slice_model_kwargs_for_batch(self, model_kwargs, idx, batch_size):
    ...
    def _pad_sequence_batch(self, sequences, pad_token_id):
        max_len = max(seq.shape[1] for seq in sequences)
        padded = []
        for seq in sequences:
            if seq.shape[1] < max_len:
                pad = seq.new_full((seq.shape[0], max_len - seq.shape[1]), pad_token_id)
                seq = torch.cat([seq, pad], dim=1)
            padded.append(seq)
        return torch.cat(padded, dim=0)

    def _trim_left_padding_for_single(self, input_ids, model_kwargs):
        ...
        nonzero = torch.nonzero(attention_mask[0], as_tuple=False)
        if nonzero.numel() == 0:
            return input_ids, model_kwargs
        ...
        trimmed_kwargs = {}
        for key, value in model_kwargs.items():
            if isinstance(value, torch.Tensor) and value.ndim >= 2 and value.shape[0] == 1 and value.shape[-1] == input_ids.shape[1]:
                trimmed_kwargs[key] = value[..., start:]
            else:
                trimmed_kwargs[key] = value
        return input_ids[:, start:], trimmed_kwargs
    ...
    def generate(
        ...
        **kwargs,
        ...
        skip_batch_fallback = bool(kwargs.pop("_skip_batch_fallback", False))
        ...
        if generation_config is None:
            ...
            has_non_default_generation_parameters = False
            if hasattr(self.config, "_has_non_default_generation_parameters"):
                try:
                    has_non_default_generation_parameters = self.config._has_non_default_generation_parameters()
                except Exception:
                    has_non_default_generation_parameters = False
            if (
                self.generation_config._from_model_config
                and self.generation_config._original_object_hash == hash(self.generation_config)
                and has_non_default_generation_parameters
            ):
        ...
        if not hasattr(generation_config, "_eos_token_tensor"):
            ...
            if eos_token_id is None:
                ...
                generation_config._eos_token_tensor = torch.tensor(eos_token_id, device=self.device)
            else:
                generation_config._eos_token_tensor = torch.tensor([eos_token_id], device=self.device)
        if not hasattr(generation_config, "_pad_token_tensor"):
            ...
            elif isinstance(pad_token_id, (list, tuple)):
                generation_config._pad_token_tensor = torch.tensor(pad_token_id, device=self.device)
            else:
                generation_config._pad_token_tensor = torch.tensor([pad_token_id], device=self.device)
        if not hasattr(generation_config, "_bos_token_tensor"):
            bos_token_id = generation_config.bos_token_id
            if bos_token_id is None:
                generation_config._bos_token_tensor = None
            elif isinstance(bos_token_id, (list, tuple)):
                generation_config._bos_token_tensor = torch.tensor(bos_token_id, device=self.device)
            else:
                generation_config._bos_token_tensor = torch.tensor([bos_token_id], device=self.device)
        ...
        if hasattr(self, "_prepare_special_tokens"):
            kwargs_has_attention_mask = bool(model_kwargs.get("attention_mask", None) is not None)
        ...
        if generation_config.max_new_tokens is not None:
            generation_config.max_length = input_ids_seq_length + int(generation_config.max_new_tokens)
 
        do_sample = kwargs.get('do_sample', False)
        max_len = kwargs.get('max_length', 200)
        top_p = kwargs.get('top_p', 1.0)
        temperature = kwargs.get('temperature', 1.0)
        batch_size = int(inputs_tensor.shape[0]) if inputs_tensor.ndim > 1 else 1
        wants_dict = bool(kwargs.get('return_dict_in_generate', False)) or bool(
            getattr(generation_config, "return_dict_in_generate", False)
        )
        batch_fallback_enabled = os.environ.get("CONTRAST_BATCH_FALLBACK", "1") != "0"

        # The single-sample path is validated; use it for batch requests to avoid corrupted contrast decoding.
        if batch_size > 1 and batch_fallback_enabled and not skip_batch_fallback and not wants_dict:
            ...
            for idx in range(batch_size):
                per_sample_kwargs = self._slice_model_kwargs_for_batch(model_kwargs, idx, batch_size)
                per_sample_inputs = inputs_tensor[idx:idx + 1]
                per_sample_inputs, per_sample_kwargs = self._trim_left_padding_for_single(
                    per_sample_inputs,
                    per_sample_kwargs,
                )
                seq = self.generate(
                    inputs=per_sample_inputs,
                    generation_config=copy.deepcopy(generation_config),
                    ...
                    _skip_batch_fallback=True,
                    **per_sample_kwargs,
                )
                sequences.append(seq)
            return self._pad_sequence_batch(sequences, int(pad_token_id))
    ...
    def _ensure_cache_position(self, input_ids, model_kwargs):
        if not isinstance(model_kwargs, dict):
            return model_kwargs
        ...
        if past_key_values is not None:
            try:
                start = int(past_key_values[0][0].shape[-2])
                # Transformers 4.38 expects positions only for the tokens that
                # will be forwarded after slicing by prepare_inputs_for_generation.
                # When input_ids is the full generated prefix, that is [start, seq_len).
                # When input_ids already contains only new tokens, use [start, start + seq_len).
                stop = seq_len if seq_len > start else start + seq_len
    ...
    def greedy_search(
        ...
        **model_kwargs,
        ...
        disable_cache = os.environ.get("DISABLE_CONTRAST_CACHE", "0") == "1"
        ...
        model_kwargs_assist = self._ensure_cache_position(input_ids, model_kwargs_assist)
        ...
        while True:
            ...
            if disable_cache:
                ...
                model_kwargs["attention_mask"] = torch.ones_like(input_ids, device=input_ids.device)
                model_kwargs_assist.pop("past_key_values", None)
                model_kwargs_assist.pop("cache_position", None)
                model_kwargs_assist["attention_mask"] = torch.ones_like(input_ids, device=input_ids.device)
            ...
            strict_top_mask = os.environ.get("CONTRAST_STRICT_TOP_MASK", "1") != "0"
            ...
            # argmax
            if self.top_logit_filter > 0.0 and strict_top_mask:
    ...
    def sample(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            #? Apply relative top filter
            if self.top_logit_filter > 0.0:
                next_token_logits, mask, _ = self.relative_top_filter(next_token_logits, relative_top=self.top_logit_filter)
                base_logits = assit_next_token_logits.log_softmax(dim=-1) #* Why log softmax a logit?
                # mask = next_token_logits[0] < -1e3
                # base_logits[0][mask] = -1e3
                base_logits[mask] = 0
                assit_next_token_logits = base_logits
