...
class ContrastLLM(torch.nn.Module, ContrastGenerationMixin):
    def __init__(self, basellm : AutoModelForCausalLM, assist_llm : AutoModelForCausalLM, weight : float, top_logit_filter=0.0) -> None:
        ...
        self.weight = weight
        self.base_device = next(self.basellm.parameters()).device
        self.assist_device = next(self.assist_llm.parameters()).device
    ...
    def forward(
        ...
        assist_logits = assit_outputs.logits.to(self.base_device)
        ...
        loss = None
        loss = self.get_loss(logits, _mv(labels, self.base_device), _mv(attention_mask, self.base_device))
